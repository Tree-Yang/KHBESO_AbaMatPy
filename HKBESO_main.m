...
while ObjChange10 > ConvergeCondition
    ...
    if IterNum == 0
        ...
        NodConnectNumF = repmat(sum(NodeEleConnect,2),1,length(ElementF(:,1)));
        ...
        %从单元灵敏度到节点灵敏度的过滤矩阵
        EleSen2NodeM = (1 - NodEleDispC ./ NodEleDispCSF) ./ (NodConnectNumF - 1);
        clear NodConnectNumF;

        %从节点灵敏度到单元灵敏度的过滤矩阵
        NodEleDispC = max(Rmin-NodEleDispC, 0);
        NodEleDispCSF = repmat(sum(NodEleDispC,2),1,length(ElementF(:,1)));
        NodeSen2EleM = (NodEleDispC ./ NodEleDispCSF)';
    end

    %当前循环中目标体积分数
    if VolFrac > VolFracCon
        TargetVolFrac = VolFrac * (1 - EvoluationRate);
    elseif VolFrac < VolFracCon
        TargetVolFrac = VolFrac * (1 + EvoluationRate);
    else
    ...
    % EleSensitivity = EleElasEnerDen;
    EleSensitivity = load('EleElasEnerDen.dat');
    ...
    EleSenCr = OrdEleSenFiltered(round(length(ElementF(:,1)) * TargetVolFrac) - 1);
